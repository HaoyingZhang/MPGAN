...
class Generator(nn.Module):
    def __init__(self, input_dim=2, latent_dim=None, hidden_dim=64, output_length=100):  # output_length = n
        super().__init__()
        self.latent_dim = latent_dim
        input_size = input_dim + (latent_dim if latent_dim is not None else 0)
        self.lstm = nn.LSTM(
        ...
        self.fc = nn.Linear(hidden_dim * 2, output_length, drop_out=0.05)

    def forward(self, mp_input, z=None):  # mp_input: (B, n-m+1, 2)
        if self.latent_dim is not None and z is not None:
            # Expand z to all time steps
            B, seq_len, _ = mp_input.shape
            z_expanded = z.unsqueeze(1).expand(-1, seq_len, -1)  # (B, seq_len, latent_dim)
            mp_input = torch.cat([mp_input, z_expanded], dim=-1)  # (B, seq_len, input_dim + latent_dim)
...
class Discriminator(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super().__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, batch_first=True, num_layers=20)
